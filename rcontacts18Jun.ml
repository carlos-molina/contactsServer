...
(**
 ...
 qry:    query
 *)
let rresults policyQry qry= (Rete.exec_qry policyQry (List.flatten contacts)) |> (Rete.exec_bm qry)
...
let getWhole policyQry qry= 
    let results= (Rete.exec_qry policyQry (List.flatten contacts)) |> (Rete.exec_bm qry) in
...
let get_contactDataByName policyQry qry tupLst varsLst= 
  ...
  let r_map= Rete.get_res_map results varsLst (* [ "?name"; "?y"; "?email" ] *)           in
  print_string("\n\n The result of getWhole map is: \n\n");
  Contactshelper.mapLstToHtml_ul_Str (Helper.StringMap.bindings r_map)
...
let kontacts= List.map (fun str -> Helper.to_tuple_lst str) Contactsdata.kontacts 

let getContactDataByFirstName firstName= 
  match firstName with
  | ""  -> Contactshelper.listoftuples_to_html_str kontacts 
  | _   ->
    ...
    let results= (Rete.exec_qry Contactsdata.defaultPlcy (List.flatten kontacts)) 
                 |> (Rete.exec_bm qry) in
...
let getContactDataByLastName lastName= 
  match lastName with
  | ""  -> Contactshelper.listoftuples_to_html_str contacts 
  | _   ->
...
(**
 ...
 @date:   5 Jan 2015, Computer Laboratory, Univ. of Cambridge
 *)
let getStrOfTuples() = Contactshelper.listoftuples_to_str contacts
...
let getHtmlStringOfTuples tupLst= 
  let contactsLst= List.map (fun str -> Helper.to_tuple_lst str) tupLst in
  Contactshelper.listoftuples_to_html_str contactsLst
